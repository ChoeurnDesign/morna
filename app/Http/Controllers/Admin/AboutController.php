...
{
    $request->validate([
        'title' => 'nullable|string|max:255', // Changed from required to nullable
        'subtitle' => 'nullable|string|max:500',
        'story_text' => 'nullable|string', // Changed back to nullable
    ...
    public function updateTeamMember(Request $request, TeamMember $teamMember)
    {
        $request->validate([
            ...
            'role' => 'required|string|max:255', // Changed from 'position'
            'bio' => 'required|string|max:1000', // Changed from 'description'
            'sort_order' => 'integer',
            'is_active' => 'boolean',
            'image' => 'nullable|image|mimes:jpeg,png,jpg|max:1024',
        ]);

        $data = $request->only(['name', 'role', 'bio', 'sort_order', 'is_active']); // Changed fields
    ...
    public function storeTeamMember(Request $request)
    {
        ...
        $data = $request->only(['name', 'role', 'bio', 'sort_order']); // Changed fields
        $data['is_active'] = true;
